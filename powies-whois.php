<?php
/*
Plugin Name: Powie's WHOIS Domain Check
Plugin URI: https://powie.de/wordpress/whois/
...
if( ! function_exists('pwhois_create_menu') )
{
	function pwhois_create_menu()
	{
		add_options_page(__('Powies WHOIS Setup'), __('Powies WHOIS Setup'), 'manage_options', PWHOIS_PLUGIN_DIR.'/pwhois_settings.php');
	}
	add_action('admin_menu', 'pwhois_create_menu');
}
	...
	//$sc.='<option '.$selected.' value=".de">.de</option>';
		foreach($PWHOIS_SERVERS as $ws => $v)	{
			...
			if (isset($atts['default'])) {
				$selected = ( $ws == $atts['default'] ) ? 'selected' : '';
			}
...
function pwhois_activate() {
    ...
    if ( get_option('display-on-free') == "") {
        update_option('display-on-free',__('Domain is free'));
    }
    if ( get_option('display-on-connect') == "") {
        update_option('display-on-connect',__('Domain is not free'));
    }
}
...
*/
if( ! function_exists( 'whois_scripts' ) )
{
	function whois_scripts()
	{
		wp_enqueue_script('pwhois-ajax-request', plugin_dir_url( __FILE__ ).'pwhois.js', array( 'jquery' ) );
		// declare the URL to the file that handles the AJAX request (wp-admin/admin-ajax.php)
		wp_localize_script( 'pwhois-ajax-request', 'pWhoisAjax', array('ajaxurl' => admin_url('admin-ajax.php'), 'enter_domain' => __('Please enter Domain', 'pwhois') ) );
	}
		add_action( 'wp_enqueue_scripts', 'whois_scripts' );
}
...
function pwhois_post(){
	...
	if (! wp_verify_nonce($_POST['post_nonce'], 'pwhoisnonce') ) die('Security check');
		...
		//	$msg=get_option('display-on-free');
		} elseif (stristr($result,' is free')) {
			$msg=get_option('display-on-free');
		} elseif (stristr($result,'no match')) {
			$msg=get_option('display-on-free');
		} elseif (stristr($result,'not found')) {
			$msg=get_option('display-on-free');
		} elseif (strstr($result,'Available')) {
			$msg=get_option('display-on-free');
		} elseif (stristr($result,'no existe')) {
			$msg=get_option('display-on-free');
...
"za"                =>  array("whois.registry.net.za"),
"co.za"             =>  array("whois.co.za")
);
